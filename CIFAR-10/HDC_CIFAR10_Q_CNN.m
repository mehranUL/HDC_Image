function cl_percentage = HDC_CIFAR10_Q_CNN(image_row_size,image_column_size,D,images_train,images_test,P_hypervector...
...
%sobol_seq_new = sobol_seq_new(:,700:1023); %311  400-723-->46 700-1023-->47
%sobol_seq_new = [sobol_seq_new,vd(:,1:13)];
%sobol_seq_new = [sobol_seq_new,R2,vd(:,1:9),weyl];
%sobol_seq_new = [sobol_seq_new,weyl];
...
% out_train = featureVectorTRAIN;
% out_test = featureVectorTEST;

out_train = images_train;
out_test = images_test;

out_train = rescale(out_train,0,0.1);
out_test = rescale(out_test,0,0.1);
...
parfor TRAIN_IMAGE_INDEX = 1:total_training_images %50 will be total db size
    ...
    %for item_image=1:1:trainDB_size %iterating over training images
        ...
        for j = 1:image_row_size*image_column_size
            delta_sobol = (max(sobol_seq_new(:,j)) - min(sobol_seq_new(:,j)))/64; %40 16       192
            delta_zdchu = (max(re(:,j)) - min(abs(re(:,j))))/64; %8 16
            for zz = 1:D
        ...
        delta = (max(out_train(:,TRAIN_IMAGE_INDEX)) - min(out_train(:,TRAIN_IMAGE_INDEX)))/64; %80
        for i = 1:image_row_size*image_column_size
...
%         end
    ...
    if labels_train(TRAIN_IMAGE_INDEX) == 10
...
parfor TESTING_IMAGE_INDEX = 1:total_test_images %50 will be total db size
    ...
    %delta = (max(out_test(:,index1)) - min(out_test(:,index1)))/80;
        ...
        for j = 1:image_row_size*image_column_size
            delta_sobol = (max(sobol_seq_new(:,j)) - min(sobol_seq_new(:,j)))/64; %40
            delta_zdchu = (max(re(:,j)) - min(abs(re(:,j))))/64; %8
            for zz = 1:D
                if sobol_seq_new(zz,j)/delta_sobol <= (ceil(sobol_seq_new(zz,j))/delta_sobol + floor(sobol_seq_new(zz,j))/delta_sobol)/2
                    criteria_sobol(zz,j) = floor((sobol_seq_new(zz,j))/delta_sobol)*delta_sobol;
                else
...
%     end
    ...
    delta = (max(out_test(:,TESTING_IMAGE_INDEX)) - min(out_test(:,TESTING_IMAGE_INDEX)))/64;       %32
    for i = 1:image_row_size*image_column_size
...
%     end
    ...
    if position == (labels_test(TESTING_IMAGE_INDEX))
        accuracy = accuracy + 1;
    end
